...
import (
	"File_Storage/internal/api/storage"
	...
	services "File_Storage/internal/services/storage"
	"context"
	"errors"
	"net"
	"net/http"
...
func NewStorageApp(app *domain.BaseApp, grpcAddress, restAddress string) *StorageApp {
	return &StorageApp{
		app:         app,
...
func (app *StorageApp) Start() error {
	...
	handler := handlers.NewStorageServer(service)

	app.srv = grpc.NewServer()

	proto.RegisterStorageServiceServer(app.srv, handler)
	...
	if err := proto.RegisterStorageServiceHandlerFromEndpoint(ctx, mux, app.grpcAddress, opts); err != nil {
		return errors.New("Failed to register storage rest handler: " + err.Error())
	}
...
func (app *StorageApp) GracefulShutdown() {
	if app.srv != nil {
